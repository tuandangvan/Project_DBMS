...
namespace QLBanLapTop
{
    ...
    public partial class KhachHangUControl : UserControl
    {
        string strSQL = "";
        ...
        void loadlistview()
        {
            ...
            if (db.conn.State == ConnectionState.Open)
                db.conn.Close();
            daLV = new SqlDataAdapter("DECLARE @sdt char(10) = N'" +sdt + "';select dbo.fnc_TongTienDH(@sdt) as sumDH, dbo.fnc_TongDH(@sdt) as countDH, dbo.fnc_DHGanNhat(@sdt) as NgayMuaHang,  dbo.fnc_DHMax(@sdt) as maxDH", db.conn);
            dtLV = new DataTable();
            daLV.Fill(dtLV);
            int i = 0;
