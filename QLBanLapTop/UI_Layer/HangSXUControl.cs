...
namespace QLBanLapTop.UI_Layer
{
    ...
    public partial class HangSXUControl : UserControl
    {
        ...
        private void HangSXUControl_Load(object sender, EventArgs e)
        {
            LoadData();
        }
        ...
        private void dgvHSX_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (r >= 0)
            {
                ...
                mahsx = dgvHSX.Rows[r].Cells[0].Value.ToString().TrimEnd();
        ...
        {
            if (mahsx != "" && txtTenHang.Text.Trim() != "" && txtNoiSX.Text.Trim() != "" && txtWebsite.Text.Trim() != "")
            {
                DialogResult check = MessageBox.Show("Bạn có muốn Sửa Hãng Sản Xuất" + " " + mahsx, "Thông báo",
                ...
                parameters.Add(new SqlParameter("@MaHang", mahsx));
                parameters.Add(new SqlParameter("@NoiSX", txtNoiSX.Text));
                parameters.Add(new SqlParameter("@TenHang", txtTenHang.Text));
                parameters.Add(new SqlParameter("@Website", txtWebsite.Text));
